  ...
  return (
    <section className="space-y-6">
      <header className="rounded-3xl border border-slate-200 bg-white px-6 py-6 shadow-panel">
        ...
        <p className="mt-3 max-w-3xl text-sm text-slate-500">Upgrade or downgrade between curated plans now. The API client already normalizes user-friendly errors so the real backend can slot in later without changing these screens.</p>
      </header>
      ...
      <div className="grid gap-6 xl:grid-cols-3">
        {availablePlans.map((plan) => {
          ...
          return (
            <article key={plan.id} className={clsx('flex h-full flex-col rounded-3xl border bg-white p-6 shadow-panel', plan.recommended ? 'border-brand-500 ring-1 ring-brand-200' : 'border-slate-200')}>
              ...
              <p className="mt-6 text-3xl font-semibold text-slate-950">{plan.priceMonthly}<span className="text-base font-medium text-slate-500"> / month</span></p>
              <ul className="mt-6 space-y-3 text-sm text-slate-600">
                {plan.features.map((feature) => (
                  <li key={feature.label} className="flex items-center gap-3 rounded-2xl bg-slate-50 px-4 py-3"><span className={clsx('h-2.5 w-2.5 rounded-full', feature.included ? 'bg-emerald-500' : 'bg-slate-300')} aria-hidden="true" /><span>{feature.label}</span></li>
                ))}
              </ul>
              <button type="button" disabled={isCurrent || updatePlanMutation.isPending} onClick={() => updatePlanMutation.mutate(plan.id)} className={clsx('mt-8 rounded-full px-4 py-3 text-sm font-semibold transition disabled:cursor-not-allowed disabled:opacity-60', isCurrent ? 'bg-slate-100 text-slate-500' : 'bg-brand-600 text-white hover:bg-brand-700')}>{isCurrent ? 'Current plan' : updatePlanMutation.isPending ? 'Updating…' : `Switch to ${plan.name}`}</button>
            </article>
          );
        })}
      </div>
