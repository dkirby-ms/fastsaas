  ...
  return (
    <section className="space-y-6">
      <header className="rounded-3xl border border-slate-200 bg-white px-6 py-6 shadow-panel">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h1 className="mt-3 text-3xl font-semibold text-slate-950">Review marketplace listing visibility</h1>
            <p className="mt-3 max-w-3xl text-sm text-slate-500">Open a product to inspect synced assets, audience targeting, and per-plan pricing without leaving the publisher portal.</p>
          </div>
          <ReadOnlySyncBadge />
        </div>
      </header>
      ...
      {productsQuery.data.length === 0 ? (
        <div className="rounded-3xl border border-dashed border-slate-300 bg-white px-6 py-12 text-center text-sm text-slate-500 shadow-panel">No products synced yet.</div>
      ) : (
        <div className="grid gap-6 xl:grid-cols-2">
          {productsQuery.data.map((product) => (
            <article key={product.id} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-panel">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  <p className="text-sm font-semibold uppercase tracking-[0.2em] text-brand-600">{product.productType}</p>
                  <h2 className="mt-2 text-2xl font-semibold text-slate-950">{product.alias}</h2>
                  <p className="mt-2 text-sm text-slate-500">Offer {product.externalOfferId}</p>
                </div>
                <span className="rounded-full bg-slate-100 px-3 py-1 text-sm font-semibold text-slate-700">{formatLifecycleState(product.lifecycleState)}</span>
              </div>

              <dl className="mt-6 grid gap-4 sm:grid-cols-2">
                <div className="rounded-2xl bg-slate-50 p-4">
                  <dt className="text-sm text-slate-500">Durable product ID</dt>
                  <dd className="mt-2 font-semibold text-slate-950">{product.durableProductId}</dd>
                </div>
                <div className="rounded-2xl bg-slate-50 p-4">
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                ...
                <Link href={`/publisher/products/${product.id}/audiences`} className="rounded-full border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-brand-500 hover:text-brand-700">
                  View audiences
                </Link>
              </div>
            </article>
