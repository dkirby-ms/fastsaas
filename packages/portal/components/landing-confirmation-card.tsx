...
const statusTone: Record<Subscription['status'], string> = {
  PendingActivation: 'bg-amber-100 text-amber-800',
        ...
        {isResolving && !subscription ? (
          <div className="flex min-h-[280px] items-center justify-center" role="status" aria-live="polite">
            <div className="text-center">
              ...
              <p className="mt-4 text-sm font-medium text-slate-600">Resolving your Marketplace subscription…</p>
            </div>
          </div>
        ) : resolveError ? (
          ...
          <div className="space-y-6">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <h2 className="mt-2 text-2xl font-semibold text-slate-950">{formatPlanName(subscription)}</h2>
                <p className="mt-2 text-sm text-slate-600">{getStatusMessage(subscription.status)}</p>
              </div>
            ...
            <dl className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              <div className="rounded-2xl bg-slate-50 p-4">
                ...
                <dd className="mt-2 text-lg font-semibold text-slate-950">{formatPlanName(subscription)}</dd>
              </div>
              <div className="rounded-2xl bg-slate-50 p-4">
                <dt className="text-sm text-slate-500">Seats</dt>
                <dd className="mt-2 text-lg font-semibold text-slate-950">{subscription.seats}</dd>
              </div>
              <div className="rounded-2xl bg-slate-50 p-4">
                <dt className="text-sm text-slate-500">Assigned tenant</dt>
                <dd className="mt-2 break-all text-sm font-semibold text-slate-950">{assignedTenant}</dd>
              </div>
              <div className="rounded-2xl bg-slate-50 p-4">
                <dt className="text-sm text-slate-500">Purchasing tenant</dt>
                <dd className="mt-2 break-all text-sm font-semibold text-slate-950">{purchaserTenant}</dd>
              </div>
            </dl>

            <dl className="grid gap-4 md:grid-cols-2">
              <div className="rounded-2xl border border-slate-200 p-4">
                <dt className="text-sm text-slate-500">Marketplace subscription</dt>
                <dd className="mt-2 break-all text-sm font-medium text-slate-900">{subscription.marketplaceSubscriptionId}</dd>
              </div>
              <div className="rounded-2xl border border-slate-200 p-4">
                <dt className="text-sm text-slate-500">FastSaaS subscription</dt>
                <dd className="mt-2 break-all text-sm font-medium text-slate-900">{subscription.id}</dd>
              </div>
            </dl>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <button
                ...
                className="rounded-full border border-slate-300 px-5 py-3 text-sm font-semibold text-slate-700 transition hover:border-brand-500 hover:text-brand-700 disabled:cursor-not-allowed disabled:opacity-60"
              >
                Refresh details
              </button>
            </div>
