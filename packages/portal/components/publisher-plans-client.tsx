  ...
  return (
    <section className="space-y-6">
      <header className="rounded-3xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-6 py-6 shadow-panel">
        ...
        <p className="mt-3 max-w-3xl text-sm text-slate-500 dark:text-slate-400">Keep pricing and lifecycle readiness aligned with the publisher experience. Dedicated APIs can replace this client abstraction without reworking the UI.</p>
      </header>
      ...
      <div className="grid gap-6 xl:grid-cols-3">
        {plansQuery.data.plans.map((plan) => {
          ...
          return (
            <article key={plan.id} className="rounded-3xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6 shadow-panel">
              <div className="flex items-start justify-between gap-3"><div><p className="text-sm font-semibold uppercase tracking-[0.2em] text-brand-600">{plan.id}</p><h2 className="mt-2 text-xl font-semibold text-slate-950 dark:text-slate-50">{plan.name}</h2></div><span className="rounded-full bg-slate-100 dark:bg-slate-800 px-3 py-1 text-sm font-semibold text-slate-700 dark:text-slate-200">{plan.activeSubscriptions} tenants</span></div>
              <div className="mt-6 space-y-4">
                <div><label className="mb-2 block text-sm font-medium text-slate-700 dark:text-slate-200" htmlFor={`${plan.id}-name`}>Name</label><input id={`${plan.id}-name`} value={draft?.name ?? ''} onChange={(event) => setDrafts((current) => ({ ...current, [plan.id]: { ...(current[plan.id] ?? draft), name: event.target.value } }))} className="w-full rounded-2xl border border-slate-300 dark:border-slate-600 px-4 py-3 outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-100" /></div>
                <div><label className="mb-2 block text-sm font-medium text-slate-700 dark:text-slate-200" htmlFor={`${plan.id}-description`}>Description</label><textarea id={`${plan.id}-description`} value={draft?.description ?? ''} onChange={(event) => setDrafts((current) => ({ ...current, [plan.id]: { ...(current[plan.id] ?? draft), description: event.target.value } }))} className="min-h-28 w-full rounded-2xl border border-slate-300 dark:border-slate-600 px-4 py-3 outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-100" /></div>
                <div className="grid gap-4 sm:grid-cols-2">
                  <div><label className="mb-2 block text-sm font-medium text-slate-700 dark:text-slate-200" htmlFor={`${plan.id}-price`}>Monthly price</label><input id={`${plan.id}-price`} value={draft?.priceMonthly ?? ''} onChange={(event) => setDrafts((current) => ({ ...current, [plan.id]: { ...(current[plan.id] ?? draft), priceMonthly: event.target.value } }))} className="w-full rounded-2xl border border-slate-300 dark:border-slate-600 px-4 py-3 outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-100" /></div>
                  <div><label className="mb-2 block text-sm font-medium text-slate-700 dark:text-slate-200" htmlFor={`${plan.id}-status`}>Status</label><select id={`${plan.id}-status`} value={draft?.status ?? 'draft'} onChange={(event) => setDrafts((current) => ({ ...current, [plan.id]: { ...(current[plan.id] ?? draft), status: event.target.value as PublisherPlanUpdateInput['status'] } }))} className="w-full rounded-2xl border border-slate-300 dark:border-slate-600 px-4 py-3 outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-100"><option value="active">Active</option><option value="draft">Draft</option></select></div>
                </div>
              </div>
              <ul className="mt-6 space-y-2 text-sm text-slate-500 dark:text-slate-400">{plan.features.map((feature) => <li key={feature} className="rounded-2xl bg-slate-50 dark:bg-slate-800/60 px-4 py-3">{feature}</li>)}</ul>
