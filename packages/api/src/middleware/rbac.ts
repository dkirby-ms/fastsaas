...
function normalizeRbacRole(role: string): RbacRole | undefined {
  return RBAC_ROLE_LOOKUP.get(role.trim().toLowerCase());
}

function getRecognizedRoles(roles: readonly string[]): RbacRole[] {
  return roles
...
export function isRbacRole(role: string): role is RbacRole {
  return RBAC_ROLE_SET.has(role) || normalizeRbacRole(role) !== undefined;
}
...
export function isActionAllowed(roles: readonly string[], resource: RbacResource, action: RbacAction): boolean {
  ...
  return getRecognizedRoles(roles).some((role) => getGrantedPermissions(role).includes(permission));
}
  ...
  return (req, _res, next) => {
    ...
    next(
      AppError.forbidden('You do not have permission to perform this action', {
        resource: options.resource,
        action: options.action,
        roles: getRecognizedRoles(req.context.roles)
      })
    );
  };
}
